...
app.controller("BookingController", [ '$scope', '$http','$window', function($scope, $http,$window) {
  ...
  $window.alert(response.status);
	   ...
	   $scope.getSlotList = function() {
			$window.alert("Calling to get slot times doctor"+$scope.chooseDoctor+"Appointment_Date"+$scope.Appointment_Date);
						$http({
							url : 'SlotsServlet',
							method : "GET",
							params : {"dcsource": $scope.chooseDoctor,"Appointment_Date":$scope.Appointment_Date}
						}).then(function(response) {
						...
						$scope.GetSlotTime = function () {
				            $scope.strTimeSlot = document.getElementById('TimeSlot').value;
				       };
